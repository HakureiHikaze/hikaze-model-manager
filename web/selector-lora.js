...
(function(){
  try { if (typeof window !== 'undefined' && window.top === window) { console.debug('[HikazeMM] selector-lora.js top window, skip'); return; } } catch(_){ }

  const urlParams = new URLSearchParams(location.search || '');
  const requestId = urlParams.get('requestId') || '';
  const preselectedRaw = urlParams.get('selected') || '';
  const preKeys = new Set((preselectedRaw||'').split(',').map(s=>decodeURIComponent(s).trim().toLowerCase()).filter(Boolean));

  const state = {
    selector: { on: true, kind: 'lora', requestId, selectedIds: new Set(), preKeys },
    types: [], currentType: 'lora', q: '', selectedTags: new Set(), tagsMode: 'all',
  ...
  function normalizeKey(s){ return String(s||'').replace(/\\/g,'/').trim().toLowerCase(); }
  function basename(p){ try{ const parts=String(p||'').split(/[\\\/]/); return parts[parts.length-1]||''; }catch(_){ return String(p||''); } }
  ...
  async function loadTypes(){
    ...
    state.types=rows;
    const want=['lora','loras'];
    const hit=rows.find(x=> want.includes(String(x.name||'').toLowerCase()));
    state.currentType = hit ? hit.name : (state.currentType || 'lora');
    renderTypeTabs();
  ...
  async function loadFacets(){
    const t=state.currentType || 'lora';
  ...
  async function loadModels(){
    ...
    url.searchParams.set('type', state.currentType || 'lora');
    if(state.q) url.searchParams.set('q', state.q);
    ...
    if(state.selector.preKeys && state.selector.preKeys.size){ for(const m of state.models){ const key=identityForModel(m); if(key && state.selector.preKeys.has(key)) state.selector.selectedIds.add(m.id); } }
    renderModels();
  }
  ...
  function renderModels(){
    ...
    state.models.forEach(m=>{
      ...
      const picked = state.selector.selectedIds.has(m.id);
      const pickBox = h('input',{type:'checkbox', checked: picked? '': null, onclick:(e)=>{ e.stopPropagation(); if(state.selector.selectedIds.has(m.id)) state.selector.selectedIds.delete(m.id); else state.selector.selectedIds.add(m.id); updateActionsState(); if(state.viewMode==='cards'){ renderModels(); } else { e.currentTarget.closest('.row')?.classList.toggle('picked', state.selector.selectedIds.has(m.id)); } }});
      if(state.viewMode==='cards'){
        const card=h('div',{class:'card'+(isSel?' selected':'')+(picked?' picked':''), onclick:()=>selectModel(m)});
        const badge=h('span',{class:'badge'}, m.type);
        const top=h('div',{class:'card-top'}, [badge, h('span',{style:{marginLeft:'auto'}}, pickBox)]);
  ...
  function updateActionsState(){ if(!el.confirmBtn) return; const can = state.selector.selectedIds.size>0; el.confirmBtn.disabled = !can; if(el.saveBtn) el.saveBtn.style.display='none'; if(el.revertBtn) el.revertBtn.style.display='none'; }
  ...
  function wire(){
    ...
    if(el.confirmBtn){ bind(el.confirmBtn,'click',(e)=> sendSelection(e)); document.addEventListener('keydown',(e)=>{ if(e.key==='Enter'){ e.preventDefault(); sendSelection(e); } }); }
  }

  async function boot(){ wire(); await loadTypes(); await updateAll(); }
