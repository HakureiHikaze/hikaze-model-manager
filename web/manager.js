...
(function(){
  // 仅在 iframe 内运行
  try { if (typeof window !== 'undefined' && window.top === window) { console.debug('[HikazeMM] manager.js top window, skip'); return; } } catch(_){ }
  ...
  const state = {
    ...
    originalDetail: null,
    tagCandidates: [],
    selector: { on: false, kind: null, requestId: null, selectedIds: new Set() }
  };
  ...
  function bind(target, event, handler){ if (!target) return; try { target.addEventListener(event, handler); } catch(_){} }
  function exists(node){ return !!node; }
  ...
  function sanitizeTagName(s){ return (typeof s==='string'?s:'').trim().toLowerCase().replace(/\s+/g,' '); }

  async function api(path){ const r=await fetch(path); if(!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }
  async function apiJSON(method,path,body){ const r=await fetch(path,{method,headers:{'Content-Type':'application/json'},body:JSON.stringify(body||{})}); if(!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }

  async function fetchModelById(id){ return api(`/models/${id}`); }
  function createShaRow(m){
    ...
    btn.addEventListener('click',async()=>{
      try{
        ...
        m.hash_hex=fresh.hash_hex; input.value=m.hash_hex||'—'; btn.textContent=m.hash_hex?'重算':'计算';
      }catch(err){ alert('计算失败: '+(err&&err.message?err.message:err)); }
      finally{ btn.disabled=false; }
    });
    row.append(lab,input,btn); return row;
  }
  function createCommunityLinkRow(m){
    ...
    openBtn.addEventListener('click',()=>{ const url=(m.extra&&m.extra.community_links)?String(m.extra.community_links).trim():''; if(!url) return; const hasProto=/^(https?:)?\/\//i.test(url); const target=hasProto?url:('https://'+url); window.open(target,'_blank'); });
    wrap.append(lab,input,openBtn); return wrap;
  }
  ...
  function renderTypeTabs(){
    if(!exists(el.typeTabs)) return; el.typeTabs.innerHTML='';
    state.types.forEach(t=>{
      const btn=h('button',{class:'tab'+(state.currentType===t.name?' active':''),onclick:()=>{ state.currentType=t.name; state.selectedTags.clear(); renderTypeTabs(); updateAll(); }}, `${t.name} (${t.count})`);
      el.typeTabs.appendChild(btn);
    });
  }

  async function loadFacets(){
    const effectiveType=state.currentType;
    ...
    try{ const byTypeUrl=new URL(location.origin+'/tags/by-type'); byTypeUrl.searchParams.set('type',effectiveType); const allTags=await api(byTypeUrl.pathname+'?'+byTypeUrl.searchParams.toString()); state.tagCandidates=Array.isArray(allTags)? allTags.map(t=> (typeof t==='string'?t:(t&&t.name))).filter(v=> typeof v==='string' && v.trim().length>0): []; }
    catch(_){ state.tagCandidates=Array.isArray(facets)? facets.map(f=>f&&f.name).filter(Boolean):[]; }
    renderTagDropdown(facets);
  }
  function renderTagDropdown(facets){
    if(!exists(el.tagDropdown)) return;
  ...
  async function loadModels(){
    ...
    if(state.selectedModel){ const found=state.models.find(it=> it && it.id===state.selectedModel.id); if(found) state.originalDetail=JSON.parse(JSON.stringify(found)); }
    renderModels();
  }
  ...
  function renderModels(){
    if(!exists(el.modelsContainer)) return; el.modelsContainer.className= state.viewMode==='cards'?'cards':'list'; el.modelsContainer.innerHTML=''; if(!state.models.length){ el.modelsContainer.appendChild(h('div',{class:'empty'},'无结果')); return; }
  ...
  function createTagChipsEditor(m){
    ...
    if(!Array.isArray(m.__tags)){ m.__tags=(Array.isArray(m.tags)? m.tags: []).filter(t=> t && t!==typeName); }
    ...
    function renderChips(){ box.querySelectorAll('.chip').forEach(n=>n.remove()); Array.from(tagsSet).forEach(tag=>{ const chip=h('span',{class:'chip',onclick:(e)=>{ e.stopPropagation(); }}, [ tag, h('button',{class:'chip-x',title:'移除',onclick:(e)=>{ e.preventDefault(); e.stopPropagation(); tagsSet.delete(tag); syncBack(); renderChips(); }}, '×') ]); box.insertBefore(chip, input); }); syncBack(); }
    function syncBack(){ m.__tags=Array.from(tagsSet); }
    function commitFromInput(){ const raw=input.value.trim(); if(!raw) return; const parts=raw.split(/[\s,]+/).map(sanitizeTagName).filter(Boolean); let changed=false; for(const p of parts){ if(p && !tagsSet.has(p)){ tagsSet.add(p); changed=true; } } input.value=''; if(changed) renderChips(); }
    function refreshSuggest(){ const q=input.value.trim().toLowerCase(); const cands=Array.isArray(state.tagCandidates)? state.tagCandidates: []; const filtered=cands.map(sanitizeTagName).filter(Boolean).filter(t=> !tagsSet.has(t) && (!q || t.includes(q))).slice(0,20); suggestWrap.innerHTML=''; if(!filtered.length){ suggestWrap.classList.add('hidden'); return; } filtered.forEach(t=>{ const item=h('div',{class:'chips-suggest-item',onclick:(e)=>{ e.stopPropagation(); tagsSet.add(t); syncBack(); renderChips(); input.focus(); suggestWrap.classList.add('hidden'); }}, t); suggestWrap.appendChild(item); }); suggestWrap.classList.remove('hidden'); }
    input.addEventListener('keydown',(e)=>{ if(e.key==='Enter'||e.key===' '||e.key===','){ e.preventDefault(); commitFromInput(); suggestWrap.classList.add('hidden'); } });
    ...
    box.append(input, suggestWrap); renderChips(); return box;
  }
  ...
  function createImageUploadRow(m){
    ...
    fileInput.addEventListener('change', async()=>{
      ...
      try{
        ...
        const js=await r.json(); const imageUrl=js.image_url; m.extra=m.extra||{}; m.extra.images=[imageUrl]; nameBox.value=js.file || (imageUrl.split('/').pop()||''); if(imageUrl){ el.detailImage.style.backgroundImage=`url(${imageUrl})`; } const found=state.models.find(it=>it.id===m.id); if(found){ found.images=[imageUrl]; } renderModels();
      }catch(err){ alert('上传失败: '+(err&&err.message?err.message:err)); }
      finally{ btn.disabled=false; btn.textContent=old; fileInput.value=''; }
    });
    wrap.append(lab,nameBox,btn,fileInput); return wrap;
  }
  ...
  function renderDetail(){
    ...
    el.detailName.textContent=m.name || m.path; const img=(m.images && m.images[0])? m.images[0]: null; el.detailImage.style.backgroundImage= img? `url(${img})`: '';
    ...
    el.detailFields.append(h('label',{class:'field'}, [h('span',{class:'label'}, '标签')]));
    el.detailFields.append(tagsEditor);
    el.detailFields.append(h('label',{class:'field'}, [h('span',{class:'label'}, 'Prompts')]), promptsEditor);
    updateActionsState();
  }

  function updateActionsState(){
    if(!exists(el.saveBtn) || !exists(el.revertBtn)) return;
    const has=!!state.selectedModel;
    el.saveBtn.style.display=''; el.revertBtn.style.display='';
    el.saveBtn.disabled=!has; el.revertBtn.disabled=!has;
  ...
  function createPromptsEditor(m){
    ...
    pos.rows=3; neg.rows=3; box.append(pos, neg); return box;
  }

  function revertDetail(){ if(!state.originalDetail) return; state.selectedModel=JSON.parse(JSON.stringify(state.originalDetail)); renderDetail(); }
  ...
  async function saveDetail(){
    ...
    if(add.length||remove.length){ const resp=await apiJSON('POST', `/models/${m.id}/tags`, {add, remove}); m.tags=resp.tags; }
    m.extra=m.extra||{}; const extraPayload={}; ['description','community_links','images','prompts'].forEach(k=>{ if(m.extra[k]!==undefined) extraPayload[k]=m.extra[k]; });
    if(Object.keys(extraPayload).length){ const resp2=await apiJSON('PATCH', `/models/${m.id}/extra`, extraPayload); m.extra=Object.assign({}, m.extra||{}, resp2); }
    state.originalDetail=JSON.parse(JSON.stringify(m)); await loadModels(); renderDetail(); const txt=el.saveBtn.textContent; el.saveBtn.textContent='已保存'; setTimeout(()=>{ el.saveBtn.textContent=txt; },1000);
  }
  ...
  function wire(){
    ...
    bind(el.refreshBtn,'click', async()=>{ try{ if(!el.refreshBtn) return; el.refreshBtn.disabled=true; const old=el.refreshBtn.textContent; el.refreshBtn.textContent='启动中…'; await apiJSON('POST','/scan/start',{full:false}); el.refreshBtn.textContent='已启动'; setTimeout(()=>{ if(!el.refreshBtn) return; el.refreshBtn.textContent=old; el.refreshBtn.disabled=false; updateAll(); },1000); }catch(err){ if(el.refreshBtn) el.refreshBtn.disabled=false; alert('启动扫描失败: '+err.message); } });
    bind(el.saveBtn,'click',()=>{ saveDetail().catch(err=>alert('保存失败: '+err.message)); });
    bind(el.revertBtn,'click',()=> revertDetail());
    updateActionsState();
  }
